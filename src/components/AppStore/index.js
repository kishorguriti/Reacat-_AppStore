...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, appList: appsList, searchInput: ''}

  onChangeSearch = event => {
    const {appList, searchInput} = this.state
    this.setState({searchInput: event.target.value})
    const searchFilter = appList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    this.setState({appList: searchFilter})
  }

  onChangeTab = tabId => {
    const {activeTab, appList} = this.state

    const filterdApps = appsList.filter(each => each.category === activeTab)

    this.setState({appList: filterdApps, activeTab: tabId})
  }

  render() {
    const {appList, activeTab} = this.state
    return (
      <div className="background">
        <h1 className="heading">App Store</h1>
        <div className="search_and_input_container">
          <input
            type="search"
        ...
        <ul className="TabsContainer">
          {tabsList.map(each => (
            <TabItem
              ...
              isActiveTab={each.tabId === activeTab}
            />
          ))}
        </ul>
        ...
        <ul className="apps_Container">
          {appList.map(each => (
            <AppItem appItemDetails={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
